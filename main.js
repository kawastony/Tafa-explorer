...
function audio(){if(ctx)return;ctx=new(AudioContext||webkitAudioContext);gain=ctx.createGain();gain.gain.value=+volEl.value;gain.connect(ctx.destination)}
...
function start(){audio();if(ctx.state==='suspended')ctx.resume();stop();playing=true;step=0;const p=SCORE[typeId];timer=setInterval(()=>{const s=p.p[step%p.p.length],f=p.b*2**(s/12);note(f,.55,step%2?'triangle':'sine',.04);note(f*.5,.8,'sine',.02);step++},400);playBtnEl.textContent='❚❚'}
function toggle(){if(playing){playing=false;stop();playBtnEl.textContent='▶'}else start()}
...
function upd(p){if(!p){['mTarget','mDist','mLight','mLeft'].forEach(id=>document.getElementById(id).textContent='—');return}mTargetEl.textContent=p.name;mDistEl.textContent=fmt(p.dist_ly)+' · '+p.dist_pc.toFixed(2)+' pc';mLightEl.textContent=p.dist_ly.toFixed(1)+' yr of light';mLeftEl.textContent=left(p.dist_ly)}
function chrono(){chronoTrackEl.innerHTML='';CATALOG.forEach((p,i)=>{const b=document.createElement('button');b.className='chrono-pill'+(sel===i?' on':'');b.innerHTML='<b>'+p.name+'</b> · '+fmt(p.dist_ly);b.onclick=()=>{setMode('sky');pick(i)};chronoTrackEl.appendChild(b)})}
function list(){leftListEl.innerHTML='';if(mode==='sky')CATALOG.forEach((p,i)=>{const d=document.createElement('div');d.className='item'+(sel===i?' on':'');d.innerHTML=p.name+'<small>'+fmt(p.dist_ly)+' · light left ~'+left(p.dist_ly)+' · '+(p.method||'NASA')+'</small>';d.onclick=()=>pick(i);leftListEl.appendChild(d)});else if(mode==='cosmos')[[1,'Today'],[.5,'a=0.5'],[.3,'a=0.3'],[.1,'a=0.1']].forEach(([a,l])=>{const d=document.createElement('div');d.className='item';d.innerHTML=l+'<small>lookback '+lb(a).toFixed(2)+' Gyr</small>';d.onclick=()=>{ageSliderEl.value=a;scale(a)};leftListEl.appendChild(d)});else[['35 cone','TAFA'],['Spiral','winding'],['NASA','light-years']].forEach(([t,s])=>{const d=document.createElement('div');d.className='item';d.innerHTML=t+'<small>'+s+'</small>';leftListEl.appendChild(d)})}
...
function scale(a){cpts.scale.setScalar(.6+a*1.4);mScaleEl.textContent=a.toFixed(3);mHEl.textContent=Hz(a).toFixed(1);mQEl.textContent=qO(a).toFixed(3);const L=lb(a);mLookEl.textContent=L.toFixed(2)+' Gyr';if(mode==='cosmos'){mLightEl.textContent='~'+L.toFixed(2)+' Gly path';mLeftEl.textContent=L>.01?L.toFixed(2)+' Gyr ago':'now';mDistEl.textContent='a='+a.toFixed(3);mTargetEl.textContent='Expansion slice'}}
function setMode(m){mode=m;document.querySelectorAll('.modes [data-mode]').forEach(b=>b.classList.toggle('active',b.dataset.mode===m));mModeEl.textContent=m[0].toUpperCase()+m.slice(1);skyG.visible=m==='sky';cosG.visible=m==='cosmos';lenG.visible=m==='lens';labG.visible=m==='lab';chronoEl.style.display=(m==='sky'||m==='cosmos')?'block':'none';leftTitleEl.textContent=m==='sky'?'Stars + NASA systems':m==='cosmos'?'Expansion chronology':m==='lens'?'Quadratic lens':'Lab';leftHintEl.textContent=m==='sky'?'Zodiac · bright stars · nearest worlds · light left':m==='cosmos'?'Scrub a(t) + lookback':'TAFA geometry';rightHintEl.textContent=m==='sky'?'Light-year chronology · you see the past':m==='cosmos'?'Kawastony dual a(t)/lookback':'Observatory';camera.position.set(0,m==='cosmos'?6:3,m==='lens'?16:18);controls.target.set(0,m==='lens'?4:0,0);list();toast(m[0].toUpperCase()+m.slice(1)+' mode')}
document.querySelectorAll('.modes [data-mode]').forEach(b=>b.addEventListener('click',()=>setMode(b.dataset.mode)));
ageSliderEl.oninput=e=>scale(+e.target.value);playBtnEl.onclick=toggle;volEl.oninput=e=>{if(gain)gain.gain.value=+e.target.value};
scoreBtnEl.onclick=()=>scorePanelEl.classList.toggle('open');
document.querySelectorAll('.score-panel .type').forEach(el=>el.onclick=()=>{typeId=+el.dataset.type;document.querySelectorAll('.score-panel .type').forEach(x=>x.classList.toggle('on',x===el));trackTitleEl.textContent=SCORE[typeId].l;trackSubEl.textContent=SCORE[typeId].s;scorePanelEl.classList.remove('open');playing?start():toggle()});
...
const TOUR_SEQ=[12,15,26,27,0,2,116,21,19,6,4,22,120,121]; // Sirius → Vega → Proxima → Barnard → Hamal → Pollux → TRAPPIST → Fomalhaut → Betelgeuse → Antares → Spica → Deneb → LHS 1140 → 51 Peg b
